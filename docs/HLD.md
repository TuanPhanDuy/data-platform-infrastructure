...
`cube-semantic`'s CI sends every changed cube YAML file to `semantic-service` for
...
- **Repos involved:** `cube-semantic` (model source + CI), `semantic-service` (validation
...
- **New components:** `POST /api/model/validate` (semantic-service), `cube/Dockerfile` +
  `.github/workflows/model-ci.yml` (cube-semantic), `services.cube` in the Helm chart +
...
3. **Merge to `main`** (only possible once `validate` is a required check, configured in GitHub
   branch protection — out of scope of this repo, a `cube-semantic` setting).
...
- **Auditability:** every deploy is a commit in this repo with a message linking back to the
  `cube-semantic` commit that caused it (`sync` job, see LLD.md §3) — `git log` on this repo
...
| **`INFRA_REPO_TOKEN` is a long-lived PAT, not scoped per-workflow** | Fine for now; a GitHub App installation token (auto-expiring, repo-scoped) is the production-grade replacement. |
